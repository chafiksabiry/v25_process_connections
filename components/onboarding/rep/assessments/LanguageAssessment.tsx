...
import { getPassage, analyzeRecordingVertex, uploadRecording } from '@/lib/rep-profile/assessments/api';
...
export default function LanguageAssessment({ language, onComplete }: LanguageAssessmentProps) {
  const { updateLanguageAssessment } = useProfile();
  ...
  useEffect(() => {
    ...
    const loadPassage = async () => {
      try {
        ...
        setPassageError(error.message || 'Failed to load passage');
        setPassage(null);
      } finally {
  ...
  // Analyze recording using vertex : API Google via Backend
  const analyzeAudio = async () => {
    ...
    try {
      ...
      const data = {
        "fileUri": res.data?.fileUri || res.fileUri, // Adjust based on actual response structure
