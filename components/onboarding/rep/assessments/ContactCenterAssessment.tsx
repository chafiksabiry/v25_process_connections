...
import { uploadRecording } from '@/lib/rep-profile/assessments/api'; // Reuse upload from assessments
// Need to migrate transcribeLongAudio and analyzeContentCenterSkill
import { useProfile } from '@/lib/rep-profile/hooks/useProfile';
import api from '@/lib/rep-profile/client'; // Use unified client

const openai = new OpenAI({
  apiKey: process.env.NEXT_PUBLIC_OPENAI_API_KEY,
  dangerouslyAllowBrowser: true // Ideally this should be server-side
});
...
interface ContactCenterAssessmentProps {
  saveResults: (results: any) => Promise<void>;
  onComplete: (results: any) => void;
  profileData?: any;
}

export default function ContactCenterAssessment({ saveResults, onComplete, profileData }: ContactCenterAssessmentProps) {
  ...
  // Placeholder for missing analyzeContentCenterSkill and transcribeLongAudio
  // These should be moved to backend/API calls
  const analyzeContentCenterSkill = async (data: any) => {
      // Mock or call backend
      const response = await api.post('/assessments/analyze-contact-center', data);
      return response.data; // Adjust based on backend
  }

  const transcribeLongAudio = async (data: any) => {
      // Mock or call backend
      const response = await api.post('/assessments/transcribe-audio', data);
      return response.data;
  }

  const mapScoreToProficiency = (score: number) => {
  ...
  const analyzeRecord = async () => {
    ...
    try {
      ...
      let data = {
        "fileUri": res.fileUri || res.data?.fileUri, // Check API response structure
        "scenarioData": scenario
      }
      // Note: We might want to transcribe first or let the analyze endpoint do it.
      // Assuming analyzeContentCenterSkill does it or we do it separately.
      // For now, let's assume analyzeContentCenterSkill handles it or we skip transcription display if not needed immediately.
      // But original code did transcribe.
      
      const transcriptionResult = await transcribeLongAudio({ 
          "fileUri": res.fileUri || res.data?.fileUri, 
          "languageCode": "en-US" 
      });
      ...
      const feedback = {
        ...
        skill: skill.name,
        score: response.data?.score || response.score, // Adjust based on backend response
        proficiency: mapScoreToProficiency(response.data?.score || response.score),
        strengths: response.data?.strengths || response.strengths,
        improvements: response.data?.improvements || response.improvements,
        feedback: response.data?.feedback || response.feedback,
        tips: response.data?.tips || response.tips,
        keyMetrics: response.data?.keyMetrics || response.keyMetrics
      };
  ...
  const calculateScoresPerCategory = (scores: any) => {
  ...
  const getCurrentProficiency = (category: string | undefined, skillName: string | undefined) => {
    if (!profileData?.skills?.contactCenter || !category || !skillName) return null;
    
    const skill = profileData.skills.contactCenter.find(
      (s: any) => s.category === category && s.skill === skillName
    );
                      ...
                      {skillScore && (
                        <div className="text-sm text-gray-600">
                          ...
                          <div className="flex gap-4">
                            {Object.entries(skillScore.keyMetrics).map(([metric, score]: [string, any]) => (
              ...
              <div className="space-y-6 bg-gray-50 rounded-lg p-6">
                ...
                <div className="grid gap-4">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {Object.entries(feedback.keyMetrics).map(([metric, score]: [string, any]) => (
