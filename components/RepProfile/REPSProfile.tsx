...
interface REPSProfileProps {
  assessmentResults: any;
  profileData: any;
}

export default function REPSProfile({ assessmentResults, profileData }: REPSProfileProps) {
  ...
  const handlePublishProfile = () => {
    ...
    // In Next.js, use router.push if needed, but for now just alert
  ...
  const renderScoreCard = (title: string, score: number, color: string) => {
    ...
    const bgClass = colorClasses[color]?.split(' ')[1] || 'bg-gray-50';
    const textClass = colorClasses[color]?.split(' ')[0] || 'text-gray-600';
    
    return (
        <div className={`p-4 rounded-xl ${bgClass}`}>
        <div className="text-center">
            <div className={`text-3xl font-bold mb-1 ${textClass}`}>{score}%</div>
            <div className={`text-sm font-medium ${textClass.replace('600', '800')}`}>{title}</div>
        </div>
        </div>
    );
  };

  const renderSkillBadge = (skill: string, level: number) => (
    ...
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
      ...
      <div className="h-48 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 relative">
        ...
        <div className="absolute bottom-0 left-36 p-6 text-white">
          <h1 className="text-3xl font-bold mb-2">{profileData?.personalInfo?.name || 'User Name'}</h1>
          <div className="flex items-center gap-4 text-white/90">
            <span className="flex items-center gap-1">
              ...
              {profileData?.personalInfo?.location || 'Location'}
            </span>
            <span className="flex items-center gap-1">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              {profileData?.professionalSummary?.currentRole || 'Role'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 pb-6">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {Object.entries(assessmentResults.languages || {}).map(([language, results]: [string, any]) => (
      ...
      <div className="px-6 pb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {(assessmentResults.keySkills || []).map((skill: any, index: number) => (
            <div key={`skill-${index}`}>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {(assessmentResults.keyStrengths || []).map((strength: string, index: number) => (
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {(assessmentResults.developmentAreas || []).map((area: string, index: number) => (
                    <li key={index} className="flex items-start bg-white p-3 rounded-lg">
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {(assessmentResults.trainingRecommendations || []).map((training: string, index: number) => (
                    <li key={index} className="flex items-start bg-white p-3 rounded-lg">
