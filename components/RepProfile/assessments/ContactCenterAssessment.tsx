...
export default function ContactCenterAssessment({ saveResults, onComplete, profileData }: ContactCenterAssessmentProps) {
  ...
  const generateScenario = async () => {
    ...
    try {
      ...
      const res = await api.post('/onboarding/assessments/generate-scenario', {
        skillName: skill.name,
        categoryName: currentCategory.name
      });

      console.log("scenarioData : ", res.data);
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.current.onstop = () => {
        const audioBlob = new Blob(audioChunks.current, { type: "audio/webm" }); // Or audio/opus if supported
  ...
  const analyzeRecord = async () => {
    ...
    try {
      ...
      // Ensure file type matches what Vertex expects or what recorder produced. 'audio/webm' is standard for MediaRecorder.
      // Vertex AI typically supports 'audio/webm' or 'audio/opus' depending on container.
      const file = new File([audioBlob!], `audio-${Date.now()}.webm`, { type: "audio/webm" }); 
      formData.append('file', file);
      formData.append('destinationName', `audio-${Date.now()}.webm`);
      
      const uploadRes = await api.post('/ai/vertex/audio/upload', formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
      });
      
      const fileUri = uploadRes.data.fileUri;

      // Transcribe (optional for user display, but good for UX)
      try {
          const transRes = await api.post('/ai/speech-to-text/transcribe', { fileUri });
          if (transRes.data.transcription) {
              setResponse(transRes.data.transcription);
          }
      } catch (e) {
          console.error("Transcription failed but continuing evaluation", e);
      }

      // Analyze skill
      const analyzeRes = await api.post('/ai/vertex/contact-center/evaluate', {
        fileUri: fileUri,
        scenarioData: scenario
      });

      const responseData = analyzeRes.data;
      ...
      const feedbackData = {
        ...
        proficiency: mapScoreToProficiency(responseData.score),
      ...
      setScores((prev: any) => ({
        ...prev,
        [`${currentCategory.name}-${skill.name}`]: feedbackData
      }));
    } catch (error) {
  ...
  const getCurrentProficiency = (category: string, skillName: string) => {
