...
import api from '@/lib/rep-profile/client'; // Unified client
import { Loader2 } from 'lucide-react';

interface LanguageAssessmentProps {
  language: string;
  onComplete: (data: any) => void;
}

export default function LanguageAssessment({ language, onComplete }: LanguageAssessmentProps) {
  ...
  useEffect(() => {
    ...
    const loadPassage = async () => {
      try {
        ...
        const { data } = await api.get('/onboarding/assessments/language', {
            params: { language }
        });
        setPassage(data);
      } catch (error: any) {
        console.error('Error loading passage:', error);
        setPassageError(error.response?.data?.error || error.message);
        setPassage(null);
      } finally {
  ...
  const mapScoreToCEFR = (score: number) => {
  ...
  const analyzeAudio = async () => {
    ...
    try {
      ...
      const res = await api.post('/ai/vertex/audio/upload', formData, {
          headers: {
      ...
      const response = await api.post('/ai/vertex/language/evaluate', data);
      
      // Need to parse the response if it returns raw text instead of JSON
      // Vertex service returns "text" which is hopefully JSON string
      let assessmentResults;
      try {
          // If response.data is string, parse it. If object, use it.
          // Vertex service "evaluateRepLanguage" formats it using "cleanAndFormatResponse"
          assessmentResults = typeof response.data === 'string' ? JSON.parse(response.data) : response.data;
      } catch (e) {
          // Fallback if not JSON
          console.warn("Failed to parse JSON, raw:", response.data);
          assessmentResults = { overall: { score: 0, feedback: "Analysis format error" } };
      }
