  ...
  try {
    ...
    const [basicInfoRes, experienceRes, skillsRes, achievementsRes] = await Promise.all([
        openai.chat.completions.create({
            model: "gpt-3.5-turbo",
            messages: [{ role: "system", content: basicInfoPrompt.content as string }, { role: "user", content: text }],
            temperature: 0.3,
            response_format: { type: "json_object" }
        }),
        openai.chat.completions.create({
            model: "gpt-3.5-turbo",
            messages: [{ role: "system", content: experiencePrompt.content as string }, { role: "user", content: text }],
            temperature: 0.3,
            response_format: { type: "json_object" }
        }),
        openai.chat.completions.create({
            model: "gpt-3.5-turbo",
            messages: [{ role: "system", content: skillsPrompt.content as string }, { role: "user", content: text }],
            temperature: 0.3,
            response_format: { type: "json_object" }
        }),
        openai.chat.completions.create({
            model: "gpt-3.5-turbo",
            messages: [{ role: "system", content: achievementsPrompt.content as string }, { role: "user", content: text }],
            temperature: 0.3,
            response_format: { type: "json_object" }
        })
    ]);
    ...
    return NextResponse.json(
      { message: 'Failed to analyze CV', error: error.message },
      { status: 500 }
    );
  }
}
